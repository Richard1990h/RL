...
let scannerProcess = null; // Python window scanner companion
...
const services = {
  ...
  nextjs: {
    ...
    command: NODE_EXE,
    args: [path.join(RALLY_LIVE_DIR, "node_modules", "next", "dist", "bin", "next"), "start", "-p", "4500"],
    cwd: RALLY_LIVE_DIR,
    port: 4500,
...
function launchServiceDetached(svcKey) {
  const svc = services[svcKey];
  try {
    const filePath = svc.command;
    const args = svc.args || [];

    const psArgs = args.map((a) => `'${escapePsSingle(a)}'`).join(",");
    const script = `
$wd='${escapePsSingle(svc.cwd)}'
$fp='${escapePsSingle(filePath)}'
$al=@(${psArgs})
Start-Process -FilePath $fp -ArgumentList $al -WorkingDirectory $wd -WindowStyle Minimized | Out-Null
Write-Host "OK"
`;
...
function startService(svcKey, reason) {
  ...
  try {
    ...
    addLog(svcKey, `Spawn failed: ${msg}`, "error");
    console.log(`[Watchdog] ${svc.name} spawn failed: ${msg}`);

    // Fallback: detached Start-Process (works when Node spawn is blocked by policy/EPERM).
    const launched = launchServiceDetached(svcKey);
    if (!launched) {
      svc.status = "error";
      svc.lastError = msg;
      addLog(svcKey, "Detached launch fallback failed", "error");
      return;
    }

    setTimeout(() => {
      if (svcKey === "cloudflare") {
        const pid = findExistingRallyTunnel();
        if (pid) {
          svc.status = "running";
          svc.pid = parseInt(pid);
          svc.process = null;
          svc.upSince = Date.now();
          svc.healthFailCount = 0;
          addLog(svcKey, `Started via detached fallback (PID ${pid})`, "ok");
          return;
        }
      } else if (svc.port) {
      ...
      svc.status = "error";
      svc.lastError = "Detached fallback launch did not become healthy";
      addLog(svcKey, svc.lastError, "error");
    }, 2500);
    return;
  }
...
function boot() {
  server.listen(WATCHDOG_PORT, () => {
    ...
    // Start Python window scanner companion (needed for bridge window detection)
    try {
      const scannerScript = path.join(BRIDGE_DIR, "window-scanner.py");
      if (fs.existsSync(scannerScript)) {
        scannerProcess = spawn("python", [scannerScript], {
          cwd: BRIDGE_DIR,
          stdio: "ignore",
          windowsHide: true,
        });
        scannerProcess.on("exit", () => { scannerProcess = null; });
        console.log(`  [Scanner] Window scanner started (PID ${scannerProcess.pid})`);
      }
    } catch (err) {
      console.log(`  [Scanner] Could not start window scanner: ${err.message}`);
    }
