...
Manages three services:
  ...
  2. Rally Live      (node next start -p 4500)    — port 4500
...
RALLY_LIVE_DIR = BRIDGE_DIR.parent.parent
...
def main() -> None:
    ...
    configs = [
        ...
        ServiceConfig(
            ...
            command=NODE_EXE,
            args=[str(RALLY_LIVE_DIR / "node_modules" / "next" / "dist" / "bin" / "next"), "start", "-p", "4500"],
        ...
        ServiceConfig(
            ...
            cwd=str(BRIDGE_DIR),
            port=None,
            # Use local metrics endpoint for tunnel liveness to avoid external false negatives.
            health_url="http://127.0.0.1:20241/metrics",
        ),
    ]
    ...
    server = HTTPServer(("127.0.0.1", WATCHDOG_PORT), _Handler)
