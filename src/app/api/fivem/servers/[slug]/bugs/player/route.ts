...
// PATCH — player appends a follow-up to their own bug report
export async function PATCH(
  ...
  try {
    ...
    console.error("PATCH /api/fivem/servers/[slug]/bugs/player error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}

// GET — player fetches their own bug reports (no auth, playerId required)
  ...
  try {
    ...
    const bugs = await prisma.$queryRaw<BugRow[]>`
      SELECT id, playerName, playerId, title, description, stepsToRepro, expected,
             severity, status, location, adminNotes, screenshots, createdAt
      FROM FivemBugReport
    ...
    console.error("GET /api/fivem/servers/[slug]/bugs/player error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
